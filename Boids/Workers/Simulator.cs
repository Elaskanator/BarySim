...
namespace Boids {
	public static class Simulator {
		public static QuadTree<Boid> BuildTree(object[] p) {
		...
		public static double[][] Simulate(object[] p) {
			...
			if (Parameters.QUADTREE_HYBRID_METHOD) {
				if (Parameters.SIMULATION_ENABLE_PARALLELISM) Parallel.ForEach(tree.GetLeaves(), leaf =>
				...
				if (Parameters.SIMULATION_ENABLE_PARALLELISM) Parallel.ForEach(Program.AllBoids, b =>
