...
namespace Generic.Models {
	public abstract class AQuadTree<N, E, T> : ATree<E>
	where N : IComparable<N>
	where E : IVector<N>
	where T : AQuadTree<N, E, T> {
		...
		public readonly N[] LeftCorner;
		public readonly N[] Center;
		public readonly N[] RightCorner;

		public AQuadTree(N[] corner1, N[] corner2, T parent = null) 
		: base(parent) {//make sure all values in x1 are smaller than x2 (the corners of a cubic volume)
			if (Enumerable.Range(0, corner1.Length).All(d => corner1[d].CompareTo(corner2[d]) == 0)) throw new ArgumentException("Domain has no volume");

			var orderedCornerPoints = corner1.Zip(corner2, (a, b) => new { Min = a.CompareTo(b) < 0 ? a : b, Max = a.CompareTo(b) < 0 ? b : a }).ToArray();
			this.LeftCorner = orderedCornerPoints.Select(t => t.Min).ToArray();
			this.RightCorner = orderedCornerPoints.Select(t => t.Max).ToArray();
			this.Center = corner1.Zip(corner2, (a, b) => this.Halve(this.Add(a, b))).ToArray();//average of each dimension
		}
		...
		protected abstract T NewNode(N[] cornerA, N[] cornerB, T parent);
	...
	public abstract class AQuadTreeDouble<V, T> : AQuadTree<double, V, T>
	where V : IVector<double>
	where T : AQuadTreeDouble<V, T>{
		public override int MaxDepth => 50;

		public AQuadTreeDouble(double[] corner1, double[] corner2, T parent = null)
